...
| What top padding a new page should use (`py-12`) and why it lives per-page not in `globals.css`, or why a page's parent/back navigation must be a `PageHeader` breadcrumb and never an inline `← Posts` link | [layout-system-gotchas.md](layout-system-gotchas.md) |
...
| Why one inner element of a stretched-link card (e.g. a "Read post →" CTA) is unclickable while the rest of the card navigates, or why a decorative child must not carry `relative z-10` | [styling-gotchas.md](styling-gotchas.md) |
...
| Why two ad units stack near the end of a post list, whether a list page should add its own `MainAdsBanner`, or how `PostCardList`'s `adsBlockCycle` injection owns in-list ads | [ads-placement-list-pages-gotchas.md](ads-placement-list-pages-gotchas.md) |
...
| Why an `og:image` is broken/relative in SNS previews, whether to pass `imageURL`/`url` to `CommonMeta` as relative or absolute, why `keyword` meta is ignored, what belongs in `keywords`, or why `user-scalable=no` must not be set | [meta-tags-gotchas.md](meta-tags-gotchas.md) |
| Why a `hover:bg-bg-page` (or any hover swapping toward the page background) emphasizes a chip/button in light mode but makes it vanish in dark mode, or how to write a theme-symmetric hover | [color-tokens-gotchas.md](color-tokens-gotchas.md) |

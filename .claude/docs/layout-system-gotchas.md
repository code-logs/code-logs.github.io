...
### Page entry top padding is a per-page `py-12`, NOT a container/`main` rule
...
- **Symptom:** A page reintroduces a standalone `← Posts` / `ArrowLeft` link above the heading, or bakes an arrow into a breadcrumb label (`'← About'`).
- **Why:** Issue #185 collapsed two competing patterns (inline `ArrowLeft + label` links on Categories/Tags vs. `PageHeader` breadcrumb on Licenses/Category-detail) into one: the `PageHeader` `breadcrumb` prop is the single source for parent navigation. This keeps position, styling, and `aria-current` consistent across pages.
- **Rule:** Express "up one level" via `breadcrumb={[{ label, href }, …, { label: <current> }]}` — the last (href-less) item is the current page. NEVER add a separate inline back-link component, and NEVER put `←`/arrow glyphs in a label (the renderer's `/` separator is the only divider). Top-level pages with no parent (Posts, Home) carry no breadcrumb. See the `PageHeader` breadcrumb convention below.
...
- `PageHeader` (the shared list-page `<h1>` owner) takes an optional `breadcrumb?: BreadcrumbItem[]` rendered as a `<nav aria-label="Breadcrumb"><ol>` above the title; the last (href-less) item is the current page and carries `aria-current="page"`. It is the single source for parent/back navigation (see the pitfall above). Current callers: category detail `[{ 'Categories', /categories }, { label }]`, Categories index `[{ 'Posts', /posts/1 }, { 'Categories' }]`, Tags index `[{ 'Posts', /posts/1 }, { 'Tags' }]`, Licenses `[{ 'About', /about }, { 'Licenses' }]`. The `query` (search-mode) and `breadcrumb` props are independent.

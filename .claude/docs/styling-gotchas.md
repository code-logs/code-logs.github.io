...
- **Why:** The stretched-link `::after` overlay is a *positioned* element with `z-index: auto`. Any sibling/descendant given `relative z-10` is lifted into a higher stacking layer than the overlay, so pointer events land on that child instead of the link overlay. Hover transforms/translates can create the same hit-testing trap by making a decorative child its own stacking context while the pointer is over it. The child is not itself a link, so the click is swallowed.
- **Rule:** Inside a stretched-link card, decorative children MUST NOT carry `relative z-10` (or any positive `z-index`). Keep them unpositioned so the `::after` overlay paints above them and the whole card stays one click target. If a decorative child needs a hover affordance such as `group-hover:translate-x-0.5`, add `pointer-events-none` so hit-testing still passes through to the overlay. Only lift a child above the overlay when it is a *genuinely separate* interactive control (its own link/button). Precedent: `FeaturedPostCard.tsx` (CTA, issue #186) and `PostCardGrid.tsx` (no CTA) — both leave inner content unpositioned.

### Grid/flex children overflow the viewport on long no-space tokens unless `min-w-0` + `break-words` are both present

- **Symptom:** On a narrow mobile viewport (≈320–375px) a `PostCard` (`.post-card-grid`) whose title/description/tag contains a long no-space token (URL, long compound word, long tag name) pushes the card past the viewport, producing horizontal scroll. Only *some* cards on *some* viewports reproduce it.
- **Why:** Two defaults combine. (1) Grid and flex children default to `min-width: auto`, so they refuse to shrink below their content's min-content width — and a CSS Grid flexible track (`1fr`) likewise has an *automatic minimum* of min-content. (2) Nothing in the cascade breaks long tokens (`globals.css` sets no global `overflow-wrap`/`word-break`). So an unbreakable token forces the track/child wider than the viewport. Either fix alone is insufficient: `min-w-0` lets the box shrink but the token still won't wrap; `break-words` wraps the token but a `1fr` track's min-content minimum can still blow out.
- **Rule:** For text inside `.post-card-grid` (or any grid/flex), apply BOTH: clamp the flexible track minimum with `grid-template-columns: ... minmax(0, 1fr) ...` (NOT bare `1fr`) AND give each shrinkable child `min-w-0`, AND apply `break-words` (`overflow-wrap: anywhere`) to the text-bearing element. `overflow-wrap` is inherited, so placing `break-words` on a grid-area wrapper `<a>` reaches its child text (precedent: `PostCard.tsx` title/description anchors). Use `break-words`, NEVER `break-all` (the latter breaks every word, not just overflowing tokens). For flex tag lists, the `flex-wrap` `<li>` also needs `min-w-0` so a single over-wide tag can shrink (`Tags.tsx`/`Tag.tsx`). Issue #217.
- **Rule:** NEVER collapse `.post-card-grid`'s `max-width: 800px` block to `grid-template-columns: 1fr`. That block reflows rows/areas only and inherits the outer `auto minmax(0, 1fr) auto` on purpose — the `category . publishedAt` row still spans 3 tracks, so it is not a true single-column stack and a single-column override would collapse the category/date row.

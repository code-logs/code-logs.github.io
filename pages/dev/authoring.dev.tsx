...
const AuthoringPage: NextPage = (): ReactElement => {
  ...
  const [series, setSeries] = useState<{ prevPostTitle?: string; nextPostTitle?: string } | undefined>(undefined)
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      setThumbnailAutoGenerated(Boolean(data.thumbnailAutoGenerated))
      setThumbnailGenerationError(typeof data.thumbnailGenerationError === 'string' ? data.thumbnailGenerationError : '')
      setStage('preview')
    } catch (err) {
            ...
            <Card>
              <details open>
                ...
                <div className="mt-5 space-y-4">
                  {thumbnailAutoGenerated && thumbnailMode === 'reuse' && (
                    <div className="flex items-center gap-2 text-xs text-emerald-700 dark:text-emerald-300 bg-emerald-50 dark:bg-emerald-900/30 border border-emerald-200 dark:border-emerald-800 rounded-lg px-3 py-2">
                      <span className="font-semibold">Codex 자동 생성</span>
                      <span className="text-emerald-600 dark:text-emerald-400">$imagegen으로 생성된 이미지가 적용되었습니다.</span>
                    </div>
                  )}
                  {thumbnailGenerationError && (
                    <Alert tone="error" block>
                      <div className="space-y-2">
                        <p className="font-semibold">자동 이미지 생성 실패</p>
                        <pre className="whitespace-pre-wrap font-mono text-xs">{thumbnailGenerationError}</pre>
                        <button
                          type="button"
                          onClick={() => {

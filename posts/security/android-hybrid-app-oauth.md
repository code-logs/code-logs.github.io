...
앱은 서버용 Web client ID와 요청별 nonce를 지정해 Google ID token을 요청할 수 있다. 아래 예시는 원문 nonce를 SHA-256과 Base64 URL-safe 방식으로 변환한 값을 Credential Manager에 전달한다.
...
반환된 credential에서 ID token을 얻은 뒤 원문 nonce와 함께 백엔드에 전송한다.
...
이 예시에서 ID token의 `nonce` claim은 `hashedNonce`다. 따라서 백엔드는 전달받은 `rawNonce`를 앱과 동일한 `sha256Base64Url()` 방식으로 변환한 뒤 token claim과 비교해야 한다.

```text
expectedNonce = sha256Base64Url(request.rawNonce)
verify(idToken.nonce == expectedNonce)
```

`rawNonce`와 ID token의 해시된 nonce claim을 그대로 비교하면 검증에 실패한다. nonce 변환 규칙은 앱과 서버에서 완전히 동일해야 하며, 로그인 트랜잭션에 저장한 nonce는 한 번 검증한 뒤 재사용하지 않는다.
...
| `exp` | 토큰이 만료되지 않음 |
| `nonce` | `sha256Base64Url(rawNonce)`와 token claim이 일치 |
